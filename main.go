...
func main() {
	...
	// Сброс модели фона — вызвать при смене освещения или перестановке мебели
	app.Post("/reset", func(c *fiber.Ctx) error {
		det.Reset()
		slog.Info("background model reset")
		return c.JSON(fiber.Map{"status": "background model reset"})
	})

	slog.Info("detector ready",
		"max_width", cfg.Detector.MaxWidth,
		"backsub_history", cfg.Detector.BackSub.History,
		"morph_dilate", cfg.Detector.BackSub.MorphDilateSize)
